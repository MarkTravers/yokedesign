...
#
def yokeWithPipe(xRadius, yRadius, cornerRadius, thickness, zThickness, pipeLength, pipeOR, pipeIR, yokeCollarGap, collarThickness, poleLength, poleOROutside, poleORInside, poleYokeStep, yokePipeGap, pipeCoildGap, cellsPerThickness, unvFD=''):
    ...
    cubit.subtract([cutterBody], surfaceBodyList)
    #
    # Create bottom pole piece (poleOROutside, poleORInside, poleYokeStep)
    poleIR = pipeOR + yokePipeGap
    poleThickness = poleOROutside - poleIR
    cubit.cmd('create surface rectangle width %f height %f zplane' % (poleThickness, poleLength))
    bottomPoleSweepSurfaceID = cubit.get_last_id('surface')
    bottomPoleSweepSurfaceBody = cubit.body(cubit.get_owning_body('surface', bottomPoleSweepSurfaceID))
    cubit.move(bottomPoleSweepSurfaceBody, (poleOROutside - (poleThickness/2.0), -yRadius + thickness + (poleLength/2.0), 0.0))
    cubit.cmd('sweep surface %d yaxis angle 360' % bottomPoleSweepSurfaceID)
    bottomPoleBody = cubit.body(cubit.get_owning_body('volume', cubit.get_last_id('volume')))
    #
    cubit.cmd('create surface ellipse major radius %f minor radius %f zplane' % (poleOROutside-poleORInside, poleLength-poleYokeStep))
    curveCutterSurfaceID = cubit.get_last_id('surface')
    curveCutterSurfaceBody = cubit.body(cubit.get_owning_body('surface', curveCutterSurfaceID))
    cubit.move(curveCutterSurfaceBody, (poleOROutside, -yRadius + thickness + poleLength, 0.0))
    cubit.cmd('sweep surface %d yaxis angle 360' % curveCutterSurfaceID)
    curveCutterBody = cubit.body(cubit.get_owning_body('volume', cubit.get_last_id('volume')))
    #
    cubit.subtract([curveCutterBody], [bottomPoleBody])
    bottomPoleBlockID = cubit.get_next_block_id()
    cubit.cmd('block %d body %d' % (bottomPoleBlockID, bottomPoleBody.id()))
    #
    # Create top pole piece (poleOROutside, poleORInside, poleYokeStep)
    poleIR = pipeOR + yokePipeGap
    poleThickness = poleOROutside - poleIR
    cubit.cmd('create surface rectangle width %f height %f zplane' % (poleThickness, poleLength + thickness))
    topPoleSweepSurfaceID = cubit.get_last_id('surface')
    topPoleSweepSurfaceBody = cubit.body(cubit.get_owning_body('surface', topPoleSweepSurfaceID))
    cubit.move(topPoleSweepSurfaceBody, (poleOROutside - (poleThickness/2.0), yRadius - (thickness+poleLength)/2.0, 0.0))
    cubit.cmd('sweep surface %d yaxis angle 360' % topPoleSweepSurfaceID)
    topPoleBody = cubit.body(cubit.get_owning_body('volume', cubit.get_last_id('volume')))
    #
    cubit.cmd('create surface ellipse major radius %f minor radius %f zplane' % (poleOROutside-poleORInside, poleLength-poleYokeStep))
    curveCutterSurfaceID = cubit.get_last_id('surface')
    curveCutterSurfaceBody = cubit.body(cubit.get_owning_body('surface', curveCutterSurfaceID))
    ...
    curveCutterBody = cubit.body(cubit.get_owning_body('volume', cubit.get_last_id('volume')))
    #
    cubit.cmd('create surface rectangle width %f height %f zplane' % (poleThickness-collarThickness-yokeCollarGap, thickness))
    yokeCutterSurfaceID = cubit.get_last_id('surface')
    yokeCutterSurfaceBody = cubit.body(cubit.get_owning_body('surface', yokeCutterSurfaceID))
    cubit.move(yokeCutterSurfaceBody, (pipeOR+yokePipeGap+collarThickness + (poleThickness-collarThickness-yokeCollarGap)/2.0, yRadius - thickness/2.0, 0.0))
    cubit.cmd('sweep surface %d yaxis angle 360' % yokeCutterSurfaceID)
    yokeCutterBody = cubit.body(cubit.get_owning_body('volume', cubit.get_last_id('volume')))
    #
    cubit.subtract([curveCutterBody, yokeCutterBody], [topPoleBody])
    topPoleBlockID = cubit.get_next_block_id()
    cubit.cmd('block %d body %d' % (topPoleBlockID, topPoleBody.id()))
    #
    # Create pipe
    ...
    cubit.cmd('webcut volume all in block %d %d %d %d with plane xplane offset %f noimprint nomerge' % (yokeBlockID, pipeBlockID, bottomPoleBlockID, topPoleBlockID, 0.0))
    cubit.cmd('webcut volume all in block %d %d %d %d with plane zplane offset %f noimprint nomerge' % (yokeBlockID, pipeBlockID, bottomPoleBlockID, topPoleBlockID, 0.0))
    ...
    cubit.cmd('imprint volume all in block %d %d %d %d' % (yokeBlockID, pipeBlockID, bottomPoleBlockID, topPoleBlockID))
    cubit.cmd('merge volume all in block %d %d %d %d' % (yokeBlockID, pipeBlockID, bottomPoleBlockID, topPoleBlockID))
    cubit.cmd('volume in block %d %d %d %d size %f' % (yokeBlockID, pipeBlockID, bottomPoleBlockID, topPoleBlockID, meshSize))
    cubit.cmd('volume all in block %d %d %d %d scheme auto' % (yokeBlockID, pipeBlockID, bottomPoleBlockID, topPoleBlockID))
    cubit.cmd('mesh volume all in block %d %d %d %d' % (yokeBlockID, pipeBlockID, bottomPoleBlockID, topPoleBlockID))
    #
    # Export mesh
    ...
    cubit.cmd('group "%s" add volume all in block %d %d' % ('poles', bottomPoleBlockID, topPoleBlockID))
    exportBlockID = cubit.get_next_block_id()
    cubit.cmd('block %d %s %s %s' % (exportBlockID, 'yoke', 'pipeSample', 'poles'))
...
#         exec(command)
#
yokeWithPipe(xRadius, yRadius, cornerRadius, thickness, zThickness, pipeLength, pipeOR, pipeIR, yokeCollarGap, collarThickness, poleLength, poleOROutside, poleORInside, poleYokeStep, yokePipeGap, pipeCoilGap, cellsPerThickness=5, unvFD='')
