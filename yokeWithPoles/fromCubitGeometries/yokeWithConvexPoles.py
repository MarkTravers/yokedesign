...
#
def yokeWithPipe(xRadius, yRadius, cornerRadius, thickness, zThickness, pipeLength, pipeOR, pipeIR, yokeCollarGap, collarThickness, poleLength, poleOR, yokePipeGap, pipeCoildGap, cellsPerThickness, unvFD=''):
    ...
    poleThickness = poleOR - (pipeOR + yokePipeGap)
    cubit.cmd('create surface ellipse major radius %f minor radius %f zplane' % (poleThickness, poleLength))
    ellipseSurfaceBody = cubit.body(cubit.get_owning_body('surface', cubit.get_last_id('surface')))
    cutterBrickBody = cubit.brick(poleThickness, 2.0*poleLength, 1.0)
    cubit.move(cutterBrickBody, (-poleThickness/2.0, 0.0, 0.0))
    cubit.subtract([cutterBrickBody], [ellipseSurfaceBody])
    cutterBrickBody = cubit.brick(2.0*poleThickness, poleLength, 1.0)
    cubit.move(cutterBrickBody, (0.0, -poleLength/2.0, 0.0))
    cubit.subtract([cutterBrickBody], [ellipseSurfaceBody])
    cubit.move(ellipseSurfaceBody, (pipeOR + yokePipeGap, -yRadius + thickness, 0.0))
    cubit.cmd('sweep surface %d yaxis angle 360' % cubit.get_relatives('body', ellipseSurfaceBody.id(), 'surface')[0])
    bottomPoleBody = cubit.body(cubit.get_owning_body('volume', cubit.get_last_id('volume')-2))   # -2 because 'sweep surface...' seems to not increment cubit.get_last_id('volume')
    ...
    # Collar
    cubit.cmd('create surface rectangle width %f height %f zplane' % (collarThickness, thickness + yokeCollarGap + poleLength))
    collarSweepSurfaceID = cubit.get_last_id('surface')
    collarSweepSurfaceBody = cubit.body(cubit.get_owning_body('surface', collarSweepSurfaceID))
    cubit.move(collarSweepSurfaceBody, (pipeOR + yokePipeGap + collarThickness/2.0, yRadius - 0.5*(thickness + yokeCollarGap + poleLength), 0.0))
    cubit.cmd('sweep surface %d yaxis angle 360' % collarSweepSurfaceID)
    collarBody = cubit.body(cubit.get_owning_body('volume', cubit.get_last_id('volume')))
    # Pole
    # poleThickness = poleOR - (pipeOR + yokePipeGap + collarThickness + yokeCollarGap)
    cubit.cmd('create surface ellipse major radius %f minor radius %f zplane' % (poleOR - pipeOR, poleLength))
    ellipseSurfaceBody = cubit.body(cubit.get_owning_body('surface', cubit.get_last_id('surface')))
    cutterBrickBody = cubit.brick(poleOR, 2.0*poleLength, 1.0)
    cubit.move(cutterBrickBody, (-(0.5*poleOR - (yokePipeGap + collarThickness + yokeCollarGap)), 0.0, 0.0))
    cubit.subtract([cutterBrickBody], [ellipseSurfaceBody])
    cutterBrickBody = cubit.brick(2.0*poleOR, poleLength, 1.0)
    cubit.move(cutterBrickBody, (0.0, poleLength/2.0, 0.0))
    cubit.subtract([cutterBrickBody], [ellipseSurfaceBody])
    cubit.move(ellipseSurfaceBody, (pipeOR, yRadius - thickness, 0.0))
    cubit.cmd('sweep surface %d yaxis angle 360' % cubit.get_relatives('body', ellipseSurfaceBody.id(), 'surface')[0])
    poleBody = cubit.body(cubit.get_owning_body('volume', cubit.get_last_id('volume')-2))   # -2 because 'sweep surface...' seems to not increment cubit.get_last_id('volume')
    # Block the collar and pole
    topPoleBlockID = cubit.get_next_block_id()
    cubit.cmd('block %d body %d %d' % (topPoleBlockID, collarBody.id(), poleBody.id()))
    #
    # Create pipe
...
#         exec(command)
#
yokeWithPipe(xRadius, yRadius, cornerRadius, thickness, zThickness, pipeLength, pipeOR, pipeIR, yokeCollarGap, collarThickness, poleLength, poleOR, yokePipeGap, pipeCoilGap, cellsPerThickness=5, unvFD='')
cubit.cmd('delete volume all in block 1 4')
